...
class newSym(hash': Number) -> Sym {
  method customHash -> Number {
    hash'
  }
}
...
class newPlanner -> Planner {
  ...
  method constraintsConsuming(v: Variable) do(aBlock: Invokable) -> Done {
    def determiningC: Constraint = v.determinedBy
    v.constraints.forEach { c: Constraint ->
      ((c == determiningC).or { !c.isSatisfied }).ifFalse {
        aBlock.value(c)
...
method plannerChainTest(n: Number) -> Done {
  ...
  newStayConstraint(vars.last, SymStrongDefault, planner)
...
method plannerProjectionTest(n: Number) -> Done {
  ...
  1.asInteger.to(n)do{ i: Number ->
    ...
    dests.append(dst)
    newStayConstraint(src, SymDefault, planner)
...
class newStrength(symVal: Sym) -> Strength {
  ...
  method weakest(aStrength: Strength) -> Strength {
...
class newAbstractConstraint(strengthSymbol: Sym) -> Constraint {
  ...
  def strength: Strength = strengthOf(strengthSymbol)
  ...
  method addConstraint(planner: Planner) -> Done {
  ...
  method destroyConstraint(planner: Planner) -> Done {
  ...
  method inputsDo(aBlock: Invokable) -> Done {
    // Assume that I am satisfied. Evaluate the given block on all my current
    // input variables.
    error("AbstractConstraint subclassResponsibility")
  }

  method inputsKnown(mark: Number) -> Boolean {
  ...
  method satisfy(mark: Number) propagate(planner: Planner) -> Constraint {
...
class newBinaryConstraint(var1: Variable, var2: Variable, strengthSymbol: Sym, planner: Planner) -> Constraint {
  ...
  method inputsHasOne(aBlock: Invokable) -> Boolean {
    (direction == forward).
        ifTrue  { aBlock.value(v1) }
        ifFalse { aBlock.value(v2) }
  }
...
class newUnaryConstraint(aVariable: Variable, strengthSymbol: Sym, planner: Planner) -> Constraint {
  inherit newAbstractConstraint(strengthSymbol)
...
class newEditConstraint(aVariable: Variable, strengthSymbol: Sym, planner: Planner) -> Constraint {
  inherit newUnaryConstraint(aVariable, strengthSymbol, planner)
...
class newEqualityConstraint(var1: Variable, var2: Variable, strengthSymbol: Sym, planner: Planner) -> Constraint {
  inherit newBinaryConstraint(var1, var2, strengthSymbol, planner)
...
class newScaleConstraint(srcVar: Variable, scaleVar: Variable,
                         offsetVar: Variable, dstVar: Variable,
                         strengthSymbol: Sym, planner: Planner) -> Constraint {
  inherit newBinaryConstraint(srcVar, dstVar, strengthSymbol, planner)
  ...
  method execute -> Done {
    ...
    (direction == forward).
      ...
      ifFalse { v1.value(((v2.value - offset.value) / scale.value).asInteger) }
  }
...
class newStayConstraint(aVariable: Variable, strengthSymbol: Sym, planner: Planner) -> Constraint {
  inherit newUnaryConstraint(aVariable, strengthSymbol, planner)
...
method newVariableValue(aValue: Number) -> Variable {
